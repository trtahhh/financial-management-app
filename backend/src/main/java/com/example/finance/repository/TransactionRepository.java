...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
 ...
 // Count transactions by user and category ID (using proper entity relationship)
 @Query("SELECT COUNT(t) FROM Transaction t WHERE t.user.id = :userId AND t.category.id = :categoryId AND t.isDeleted = false")
 ...
 @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.user.id = :userId AND t.category.name = :category AND t.date BETWEEN :startDate AND :endDate AND t.isDeleted = false")
 Double sumByUserIdAndCategoryAndDateBetween(@Param("userId") Long userId, @Param("category") String category, @Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);
 ...
 @Query("SELECT t FROM Transaction t LEFT JOIN FETCH t.category LEFT JOIN FETCH t.wallet WHERE t.user.id = :userId AND t.date BETWEEN :startDate AND :endDate ORDER BY t.createdAt DESC")
 List<Transaction> findByUserIdAndDateBetweenWithEagerLoad(@Param("userId") Long userId,
 @Param("startDate") LocalDate startDate,
 @Param("endDate") LocalDate endDate);
