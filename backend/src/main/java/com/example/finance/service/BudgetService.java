...
public class BudgetService {
 ...
 log.info("Calculating spent amount for budget: categoryId={}, month={}, year={}", 
 budget.getCategoryId(), budget.getMonth(), budget.getYear());
 
 // Tính tổng chi tiêu của category cụ thế
 ...
 // Nếu category này không có chi tiêu nào, cộng thêm các giao dịch khác cùng tháng
 // Để hiển thị tổng chi tiêu chung cho tháng
 if (usedAmount.compareTo(BigDecimal.ZERO) == 0) {
 usedAmount = budgetCalculationService.calculateTotalSpentAmount(
 userId, budget.getMonth(), budget.getYear());
 log.info("Category {} has no spending, using total spent: {}", budget.getCategoryId(), usedAmount);
 }
 
 // Set spent amount
 budget.setSpentAmount(usedAmount);
 log.info("Spent amount calculated: {}", usedAmount);
 ...
 */
 ...
 return budgets.stream().map(budget -> {
 // ✅ FIX: Tính chi tiêu cho CATEGORY CỤ THỂ trong khoảng thời gian, không phải tổng
 BigDecimal actualSpent = budgetCalculationService.calculateSpentAmountByDateRange(
 ...
 result.put("categoryId", budget.getCategory() != null ? budget.getCategory().getId() : 0);
 } catch (Exception e) {
