 ...
 */
 ...
 /**
  * Endpoint mới để lấy tổng số dư thực tế từ các ví (không phụ thuộc vào khoảng ngày)
  * Endpoint này luôn trả về tổng số dư hiện tại của tất cả các ví
  */
 ...
 System.out.println("Getting current balance for userId: " + userId);
 
 // Lấy tổng số dư từ các ví
 Map<String, Object> response = dashboardService.getCurrentWalletBalance(userId);
 
 return ResponseEntity.ok(response);
 
 } catch (Exception e) {
 System.err.println("Error in getCurrentBalance: " + e.getMessage());
 e.printStackTrace();
 
 Map<String, Object> errorResponse = new HashMap<>();
 errorResponse.put("error", "Failed to get current balance");
 errorResponse.put("totalBalance", 0);
 errorResponse.put("walletCount", 0);
